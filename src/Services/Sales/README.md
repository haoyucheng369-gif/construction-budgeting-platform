...
This implements the inward dependency rule of Clean Architecture and the ports/adapters boundary of Hexagonal Architecture. Domain now contains the immutable `Quotations/Quantity` value object: a decimal value strictly greater than zero, with value equality and no rounding. It represents the numeric quantity in a work item's unit; unit association belongs to the upcoming quote-line model. `Quotations/Money` holds a decimal amount and an EUR-only currency code, allows zero/negative amounts, and preserves precision on construction. `RoundToCents()` returns a new amount using midpoint rounding away from zero. Quote and QuoteLine remain planned; QuoteLine will enforce non-negative sales prices and round final line amounts. These architecture styles and DDD are not separate framework installations.
